...
@section('main-content')
   ...
   <div class='row'>
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
            <h3>Crear Liquidacion Salario Varible</h3>
            @if (count($errors)>0)
            ...
            {!!Form::open (array('url'=>'/spliquidacionSV','method'=>'POST','autocomplete'=>'off')) !!}
            {{Form::token()}}
            <div class="form-group">
                ...
                <option value="{{$empleado->ID_EMPLEADO}}_{{$empleado->Salario_6}}_{{$empleado->Nombre_Cargo}}_{{$empleado->CEDULA}}_{{$empleado->VACACIONES_DISPONIBLES}}_{{$empleado->FECHA_INGRESO}}_{{$empleado->Salario_5}}_{{$empleado->Salario_4}}_{{$empleado->Salario_3}}_{{$empleado->Salario_2}}_{{$empleado->Salario_1}}">{{$empleado->Cod_Empleado}} {{$empleado->Empleado}}</option>
                @endforeach
                </select>
            </div>  
            <div class="form-group">
            ...
            <div>
                ...
                <input type="text" id="Dias_vacaciones" required name="Dias_vacaciones" class="form-control" >
            </div>
            <div>
            ...
            <div>
                <label for="Salario">Salario</label>
                <input type="text" id="Salario_2" required name="Salario_2" value="0" class="form-control">
            </div>
            <div>
                <label for="Salario">Salario</label>
                <input type="text" id="Salario_3" required name="Salario_3" value="0" class="form-control">
            </div>
            <div>
                <label for="Salario">Salario</label>
                <input type="text" id="Salario_4" required name="Salario_4"value="0" class="form-control">
            </div>
            <div>
                <label for="Salario">Salario</label>
                <input type="text" id="Salario_5" required name="Salario_5"value="0" class="form-control">
            </div>
            <div>
                <label for="Salario">Salario</label>
                <input type="text" id="Salario_6" required name="Salario_6"value="0" class="form-control">
            </div>
            <div>
...
@push ('scripts')
            ...
            $("#Salario_1").val(datosvalor[10]);
            $("#Salario_2").val(datosvalor[9]);
            $("#Salario_3").val(datosvalor[8]);
            $("#Salario_4").val(datosvalor[7]);
            $("#Salario_5").val(datosvalor[6]);
            $("#Fecha_Inicio").val(datosvalor[5]);
            $("#Dias_vacaciones").val(datosvalor[4]);
			$("#CEDULA").val(datosvalor[3]);
			$("#Cargo").val(datosvalor[2]);
			$("#Salario_6").val(datosvalor[1]);
