...
function  verinfo_Empleado(arg){


	//var urlraiz=$("#url_raiz_proyecto").val();
//	var miurl =urlraiz+"/form_editar_empleado/"+arg+""; 
var miurl ="form_editar_empleado/"+arg+"";
	$("#capa_modal").show();
	$("#capa_formularios").show();
	var screenTop = $(document).scrollTop();
	$("#capa_formularios").css('top', screenTop);
...
if(listado==1){ var url = "listado_empleados"; }
...
function cerrarmodal(arg){
   var urlraiz=$("#url_raiz_proyecto").val();
   $("#capa_modal").hide(); 
   $("#capa_formularios2").hide();
   	$("#capa_formularios2").html("");
...
function cargar_formularioEmpleado(arg){
   var urlraiz=$("#url_raiz_proyecto").val();
   $("#capa_modal").show(); 
   $("#capa_formularios2").show();
   var screenTop = $(document).scrollTop();
   $("#capa_formularios2").css('top', screenTop);
   $("#capa_formularios2").html($("#cargador_empresa").html());
   if(arg==1){ var miurl=urlraiz+"/form_nuevo_empleado"; }
   if(arg==2){ var miurl=urlraiz+"/form_nuevo_rol"; }
   if(arg==3){ var miurl=urlraiz+"/form_nuevo_permiso"; }
    ...
    {
     $("#capa_formularios2").html(resul);
   
    }).fail( function() 
   {
    $("#capa_formularios2").html('<span>...Ha ocurrido un error, revise su conexión y vuelva a intentarlo...</span>');
   }) ;
...
$(document).on("submit",".formempleado",function(e){
  ...
  var formu=$(this);
  var varurl="";
  if(quien=="f_crear_empleado"){  var varurl=$(this).attr("action");  var div_resul="capa_formularios";  }
  if(quien=="f_editar_empleado"){  var varurl=$(this).attr("action");  var div_resul="notificacion_E2";  }
